import { AccelSample, LegKind, RideStartEvent } from './types';
...
export function detectRideStarts(
  legs: LegLite[],
  accel: AccelSample[],
): RideStartEvent[] {
  const events: RideStartEvent[] = [];
  for (let i = 0; i < legs.length - 1; i++) {
    const a = legs[i], b = legs[i + 1];
    // Runs before step-4 reclassification, so a still leg is still 'stopped'.
    if (a.kind !== 'stopped' || b.kind !== 'moving') continue; // vehicle only
    ...
    let accelScore: number | null = null;
