...
exports.classifyStill = classifyStill;
...
const C = config_1.CONFIG.waiting;
const MOVEMENT = new Set(['walking', 'moving']);
function classifyStill(legs, i, positions, rideStartAtMs) {
    ...
    if (!(prevMove && nextMove))
        return { kind: 'stopped', confidence: null };
    ...
    const driftScore = radius == null ? null : clamp01((C.driftRampM.bad - radius) / (C.driftRampM.bad - C.driftRampM.ok));
    let base = driftScore == null
    ...
    if (followedByRide)
        base = Math.min(1, base + C.rideFollowBoost);
